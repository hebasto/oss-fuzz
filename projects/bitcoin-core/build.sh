...
export CFLAGS="-DBOOST_MULTI_INDEX_ENABLE_SAFE_MODE $CFLAGS"
export CXXFLAGS="-DBOOST_MULTI_INDEX_ENABLE_SAFE_MODE $CXXFLAGS"
...
(
  ...
  make HOST=$BUILD_TRIPLET DEBUG=1 NO_QT=1 NO_BDB=1 NO_ZMQ=1 NO_UPNP=1 NO_NATPMP=1 NO_USDT=1 \
       ...
       CFLAGS="$CFLAGS" CXXFLAGS="$CXXFLAGS" LDFLAGS="$LDFLAGS" -j$(nproc)
)
...
if [ "$SANITIZER" = "memory" ]; then
  EXTRA_BUILD_OPTIONS="-DHARDENING=OFF -DASM=OFF"
fi
cmake -S .. \
  --toolchain depends/${BUILD_TRIPLET}/share/toolchain.cmake \
  -DCMAKE_CXX_FLAGS_RELWITHDEBINFO="-g" \
  -DFUZZ=ON \
